...
legend({'ADC', 'No ADC'}, 'Location', 'SouthEast');
title([ptype ' with ADC Null Value vs \lambda/\lambda0'])
...
xlabel('\lambda/\lambda_0')
ylabel('Null Shift [\lambda/D]')
txt = ['Shift Slope no ADC p = ' num2str(py_1Copy(1)) newline 'Shift Slope ADC p = ' num2str(py_1(1))]; %['y trend p value: ' num2str(py_1) newline 'x trend p value: ' num2str(px)];
text(0.9,0.05,txt);
grid on
...
% end
% 
% inpar.lambdaOverD = lambda0Fnum_samp;
% 
% 
% %-- Null shift plots for X, Y, and the trendlines that result
% figure();
...
% 
% py_1 = polyfit(inpar.lambdas/inpar.lambda0,Yshift'/inpar.lambdaOverD,1);
% pyyw = polyval(py_1,inpar.lambdas/inpar.lambda0);
% subplot(2,2,4);
% plot(inpar.lambdas/inpar.lambda0,pyyw,'-o','Color','g');
% title('Y Offset Trend')
% txt = ['p value: ' num2str(py_1)];
% text(mean(inpar.lambdas/inpar.lambda0),mean(px),txt);
% 
