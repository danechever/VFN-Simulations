...
%     disp(getWedgeTilt(wedge_mat, wedge, 1e6*wavelength)/890.16);
    disp(getWedgeTilt(wedge_mat, wedge, 1e6*wavelength));
...
%     %%fix this
%     
% %     lam0OverD_rad*lambda0/wavelength)...
...
%         *lambdaOverD_pixels*Ycoords/N - 2*pi*p_val*lambdaOverD_pixels*Ycoords/N;
    ...
    %MFT Implementation
    OUT = 2*pi*(1/890.16*getWedgeTilt(wedge_mat,wedge,1e6*wavelength)/lam0OverD_meters*lambda0/wavelength)*Ycoords/N; % - 2*pi*p_val*lambdaOverD_pixels*Ycoords/N;
...
% 
%     phi1 = inputs.phi1_ADC; 
%     phi2 = inputs.phi2_ADC;
%     phi3 = inputs.phi3_ADC;
% 
%     clocking = inputs.clocking;
%     %tilt1 = inputs.tilt1;
%     
%     %dz = 890.16*-1*getWedgeTilt(wedge_mat, wedge, 1e6*wavelength); %2*pi*inputs.lin_offset(1)*(wavelength/inputs.lambda0) - inputs.lin_offset(2);
%     
%     dz = 0;
%     
%     %disp(getADCTilt(dz, n0, n1, n2, n3, phi1, phi2, phi3, clocking));
%     
%     OUT = -1*2*pi*(ADCTILT(dz, n0, n1, n2, n3, phi1, phi2, phi3, clocking,0)...
%         /lam0OverD_rad*lambda0/wavelength)*lambdaOverD*Ycoords/N;%...
%         %- 2*pi*p_val*lambdaOverD*Ycoords/N;
%         %-2*pi*get
